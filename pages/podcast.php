...
$pagina = 'podcast';
$titulo = 'Podcast';
$descripcion = 'Escucha los episodios del podcast de Diálogo y Desarrollo Perú sobre diálogo, desarrollo y participación ciudadana.';
require __DIR__ . '/_header.php';

$podcasts = db()->query('SELECT * FROM podcasts WHERE activo = 1 ORDER BY fecha_publicacion DESC')->fetchAll();
$podcasts = array_filter($podcasts, fn($p) => media_embed((string) ($p['url_embed'] ?? ''))[0] !== '');
?>
<style>
  .ddp-media {
    position: relative;
    ...
    <div class="container">
      <div class="row">
        <?php if (empty($podcasts)): ?>
          <div class="col-12">
            <p class="text-muted">Pronto nuevos episodios.</p>
          </div>
        <?php else: ?>
          <?php foreach ($podcasts as $i => $p): ?>
